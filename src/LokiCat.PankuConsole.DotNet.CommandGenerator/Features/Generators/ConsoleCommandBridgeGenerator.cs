...
public sealed class ConsoleCommandBridgeGenerator : IIncrementalGenerator
{
    ...
    private static ConsoleCommandInfo? GetConsoleCommandInfo(GeneratorSyntaxContext context)
    {
        ...
        var attr = methodSymbol.GetAttributes()
                               .FirstOrDefault(
                                   ad => SymbolEqualityComparer.Default.Equals(ad.AttributeClass, consoleAttrSymbol));
        ...
        var commandName = attr.ConstructorArguments.Length > 0 && attr.ConstructorArguments[0].Value is string s
            ? s
            : "";

        var parameters = methodSymbol.Parameters
                                     .Select(p => new ConsoleCommandParameter(
                                                 p.Name, p.Type.ToDisplayString(), p.HasExplicitDefaultValue,
    ...
    private static void EmitGDScript(SourceProductionContext context, ImmutableArray<ConsoleCommandInfo?> commandsRaw)
    {
        try
        {
            ...
            context.ReportDiagnostic(Diagnostic.Create(
                                         new DiagnosticDescriptor("CCBG002", "Emit",
                                                                  $"Emitting {commands.Count} commands",
                                                                  "ConsoleBridge", DiagnosticSeverity.Info, true),
                                         Location.None));

            foreach (var cmd in commands)
            {
                context.ReportDiagnostic(Diagnostic.Create(
                                             new DiagnosticDescriptor("CCBG001", "Command Found",
                                                                      $"Found: {cmd.ClassName}.{cmd.MethodName}",
                                                                      "ConsoleBridge", DiagnosticSeverity.Warning,
                                                                      true),
                                             cmd.Location));
                var baseName = cmd.CustomName.Length > 0 ? cmd.CustomName : $"{cmd.ClassName}.{cmd.MethodName}";
        ...
        {
            context.ReportDiagnostic(Diagnostic.Create(
                                         new DiagnosticDescriptor("CCBG999", "Generator Crash",
                                                                  $"Exception in ConsoleCommandBridgeGenerator: {ex.Message} Stack Trace: {ex.StackTrace}",
                                                                  "ConsoleBridge", DiagnosticSeverity.Error, true),
                                         Location.None));
        }
    }
    ...
    private static string FormatArgs(List<ConsoleCommandParameter> parameters) => string.Join(
        ", ", parameters.Select(p => p.HasDefault ? p.Name + " ?? " + FormatDefault(p.DefaultValue) : p.Name));
